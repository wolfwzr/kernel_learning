...
user1_prog_start_sector  equ 40      ;用户程序起始逻辑扇区号
user2_prog_start_sector  equ 50      ;用户程序起始逻辑扇区号
...
msg_1       db 'Now is in kernel, prepare to load user program', 0x0a, 0x0d, 0
...
salt_4      db '@ReleaseControl'
            times (256-($-salt_4))  db 0
            dd sleep_current_task
            dw kernel_code_seg_sel
...
.next_kernel_salt_item:
    ...
    mov ax,[esi+0x22]       ;设置ss0
    mov [ecx+8],ax
    mov eax,[esi+0x24]      ;设置esp0
    mov [ecx+4],eax

    mov ax,[esi+0x30]       ;设置ss1
    mov [ecx+16],ax
    mov eax,[esi+0x32]      ;设置esp1
    mov [ecx+12],eax

    mov ax,[esi+0x3e]       ;设置ss2
    mov [ecx+24],ax
    mov eax,[esi+0x40]      ;设置esp2
    mov [ecx+20],eax

    mov ax,[esi+0x10]       ;设置ldt
    mov [ecx+96],ax

    mov word [ecx+100],0    ;T=0

    mov dword [ecx+28],0    ;CR3=0

    mov word [ecx+102],104  ;设置I/O bitmap

    mov edi,[esi+0x06]      ;从TCB中获取用户程序基地址

    mov ax,[edi+0x1c]       ;设置cs
    mov [ecx+76],ax
    mov eax,[edi+0x18]      ;设置eip
    mov [ecx+32],eax

    mov ax,[edi+0x08]       ;设置ss
    mov [ecx+80],ax
    mov dword [ecx+56],0    ;设置esp

    mov ax,[esi+0x44]       ;设置ds为程序头部段
    mov [ecx+84],ax

    mov word [ecx+72],0     ;es=0
    mov word [ecx+88],0     ;fs=0
    mov word [ecx+92],0     ;gs=0

    pushfd
    pop edx
    mov dword [ecx+36],edx  ;eflags
...
.next_kernel_salt_item:
    ...
    ;加载用户程序
                                            ;用栈传参
    push dword user1_prog_start_sector       ;用户程序硬盘起始扇区号
    push ecx                                ;TCB线性基地址
    ...
    ;加载用户程序
                                            ;用栈传参
    push dword user2_prog_start_sector       ;用户程序硬盘起始扇区号
    push ecx                                ;TCB线性基地址
    call load_relocate_user_program
